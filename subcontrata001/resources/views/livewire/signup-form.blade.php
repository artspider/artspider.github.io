<div>
  <div class="registro container  mb-2 mx-auto" id="registro">
    <div class="registro--header flex justify-start mb-6 pl-4">
    ...
    <div class="registro--seleccion bg-white rounded-lg shadow-lg w-11/12 lg:w-1/2 max-w-screen-xl mb-6 mx-auto py-2">
        <p class="text-xl font-bold mt-2 mb-4 mx-4">Registra tu datos y consigue una cuenta gratis</p>
        ...
        <div class="tipo-cuenta flex justify-center">
            <input wire:model="type" type="radio" value="employer" id="employer" name="tipoCuenta">
            <label class="radio text-sm" for="employer"> Busco profesionista </label>

            <input wire:model="type" type="radio" value="expert" id="expert" name="tipoCuenta" >
            <label class="radio text-sm" for="expert"> Soy un experto </label>
        </div>
        <p class=" text-xs text-red-900 text-center ">{{ $errors->first('type') }}</p>
        <div class="datos flex flex-col items-center">
          {{-- Email --}}
          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="email"><i class="icon-envelope "></i></label>
            <input wire:model="email" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2" type="text" name="email" id="email" placeholder="Email" required/>            
          </span>
          <p class=" text-xs text-red-900 ">{{ $errors->first('email') }}</p>
          {{-- name --}}
          <span class="flex justify-center items-center w-full my-2">
              ...
              <input wire:model="name" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2" type="text" name="name" id="name" placeholder="Name" required/>
          </span>
          <p class=" text-xs text-red-900 ">{{ $errors->first('name') }}</p>
          {{-- password --}}
          <span class="flex justify-center items-center w-full my-2">
              <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="password"><i class="icon-shield"></i></label>
              <input wire:model="password" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2"  type="password" name="password" id="password" placeholder="Password" required/>
          </span>
          
          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="password"><i class="icon-shield"></i></label>
            <input wire:model="password_confirmation" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2"  type="password" name="password_confirmation" id="password_confirmation" placeholder="Confirmar password" required/>
        </span>
        <p class=" text-xs text-red-900 ">{{ $errors->first('password') }}</p>
          <span class="flex flex-col justify-center items-center w-full my-2 pl-4 pr-3">
              <p class=" text-xs font-semibold mb-2">Al hacer clic, indicas que estas de acuerdo con nuestros <a href="#">terminos y condiciones</a>.</p>
              {{-- <a href="#" class="btn btn--primary rounded-md w-full text-sm lg:text-base text-center font-bold px-10 py-4 ">Registrar</a> --}}
              <button class="btn btn--primary rounded-md w-full text-sm lg:text-base text-center font-bold px-10 py-4 " >Registrar</button>
          </span>
        </div>
        </form>
  ...
  @if ($message = Session::get('message'))
    <div class="alert text-green-100 bg-green-600 mx-auto">
